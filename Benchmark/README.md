...
side: whisper.cpp has no equivalent of this layer. The stream runs twice, once
with the encoder on the kernels and once on Core ML (the `WhisperEACP ANE`
column, where the machine has it). On Core ML a run goes through
`Whisper::transcribeAsync` and comes back on a later turn of the event loop,
which the stream pumps between ticks, so that column's `per run` and `duty` are
each run's start-to-result latency, loop turns included, rather than time the
main thread or the GPU spent on it.
