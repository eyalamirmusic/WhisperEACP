...
public:
    ...
    // And there is no third load for "the model this binary ships", because no
    // binary ships one: ModelFetch::fetch() puts the four files in
    // ModelFetch::directory() once per machine, and load(path) on that is the
    // whole of it. Which is why this class knows nothing about where a model
    // came from — it reads a directory, whoever filled it.
